...
private:
  ...
  // 基于 Pt2D 的声明
  std::vector<Pt2D> makePolylineVerticesFromLOS3D(int cam_id, const Line3D &los) const;
  void buildPolylinesOnCam(const std::vector<Line3D> &los3d, int cam_id, 
...
public:
  ...
  void computeStripIntersection(const std::vector<std::vector<Pt2D>> &polylines_px,
                                double tol_px,
                                std::vector<RowSpan> &spans) const;

  void visitPointsInRowSpans(const std::vector<RowSpan> &spans,
                             const std::vector<std::vector<Pt2D>> &polylines_px, double tol_px,
                             std::vector<int> &out_indices) const;
