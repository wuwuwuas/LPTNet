...
// 宏函数：打包 立体匹配 -> 摇晃 -> 过滤 -> 残差计算
py::tuple match_shake_and_calc_residual(
    const std::vector<Camera>& cams, 
    const std::vector<std::vector<Object2D*>>& obj2d_lists_py, 
    const ObjectConfig& obj_cfg,  // <--- 修改 1：参数类型改为 ObjectConfig
    const std::vector<Image>& current_image_list) 
{
    // Step 1: 为 StereoMatch 准备安全的 2D 粒子深拷贝
    std::vector<std::vector<std::unique_ptr<Object2D>>> obj2d_cpp;
    obj2d_cpp.reserve(obj2d_lists_py.size());
    for (const auto& cam_list : obj2d_lists_py) {
        std::vector<std::unique_ptr<Object2D>> local_list;
        local_list.reserve(cam_list.size());
        for (auto* obj : cam_list) {
            if (obj) {
                // <--- 修改 2：调用作者提供的虚函数 clone() 实现多态深拷贝
                local_list.push_back(obj->clone()); 
            }
        }
        obj2d_cpp.push_back(std::move(local_list));
    }

    // Step 2: 立体匹配 (完全在 C++ 内存中)
    StereoMatch matcher(cams, obj2d_cpp, obj_cfg); // <--- 使用 obj_cfg
    auto candidates = matcher.match();

    if (candidates.empty()) {
        return py::make_tuple(py::list(), current_image_list);
    }

    // Step 3: 位置摇晃与误差优化
    Shake shaker(cams, obj_cfg); // <--- 使用 obj_cfg
    auto flags = shaker.runShake(candidates, current_image_list);

    // Step 4: 物理过滤 (直接在底层剔除 Ghost 和 Repeated)
    std::vector<std::unique_ptr<Object3D>> valid_objs;
    valid_objs.reserve(candidates.size());
    int MASK_DROP = (int)ObjFlag::Ghost | (int)ObjFlag::Repeated;
    
    for (size_t i = 0; i < candidates.size(); ++i) {
        if (candidates[i] && !((int)flags[i] & MASK_DROP)) {
            // 只有优质粒子才能存活并转移所有权
            valid_objs.push_back(std::move(candidates[i]));
        }
    }
    ...
    // Step 5: 计算残差图像 (传入纯净的粒子列表)
    auto image_list_residue = shaker.calResidualImage(valid_objs, current_image_list, nullptr);

    // Step 6: 仅将最终存活的粒子打包回 Python (单次海关安检)
    py::list py_valid_objs(valid_objs.size());
    for (size_t i = 0; i < valid_objs.size(); ++i) {
        // py::cast 会将 C++ 对象安全转换为 Python 对象并移交所有权
        py_valid_objs[i] = py::cast(std::move(valid_objs[i]));
    }

    return py::make_tuple(py_valid_objs, image_list_residue);
}
...
#endif
      ;
        m.def("match_shake_and_calc_residual", &match_shake_and_calc_residual,
          py::arg("cams"), 
          py::arg("obj2d_lists"), 
          py::arg("obj_cfg"),  // <--- 修改 3：与函数签名保持一致
          py::arg("current_image_list"),
          "Macro function to run stereo match, shake, filter, and calculate residuals in one C++ call.");
