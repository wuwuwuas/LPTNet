...
// 替换原有的 StereoMatch::enumerateCandidatesOnCam
void StereoMatch::enumerateCandidatesOnCam(
    const std::vector<Line3D> &los3d, int target_cam,
    const std::vector<int> &chosen_cams, const std::vector<Pt2D> &chosen_pts,
    std::vector<int> &out_candidates) const {
  ...
  std::vector<IDMap::RowSpan> spans;
  int n_seg = _obj_cfg._sm_param.n_segments;
  ...
  if (n_seg <= 1) {
      ...
      std::vector<std::vector<Pt2D>> polylines_px;
      buildPolylinesOnCam(los3d, target_cam, polylines_px);
      if (polylines_px.empty()) return;

      idm->computeStripIntersection(polylines_px, _obj_cfg._sm_param.tol_2d_px, spans);
      ...
      idm->visitPointsInRowSpans(spans, polylines_px, _obj_cfg._sm_param.tol_2d_px, out_candidates);
  }
...
Line2D StereoMatch::makeLine2DFromLOS3D(int cam_id,
...
// ---- helper: make a 2D polyline vertices on camera 'cam_id' from a 3D LOS ----
// std::vector<Pt2D> StereoMatch::makePolylineVerticesFromLOS3D(int cam_id, const Line3D &los) const {
//   std::vector<Pt2D> pts_2d;
//   int n_seg = _obj_cfg._sm_param.n_segments; 

//   // 常规模式：退化为双点直线的端点
//   if (n_seg <= 1) {
//     pts_2d.push_back(_cams[cam_id].project(los.pt));
//     pts_2d.push_back(_cams[cam_id].project(Pt3D{
...
//   // 大畸变模式：在 Z 轴边界内均匀采样
//   double z_min = _obj_cfg._sm_param.limit.z_min;
//   double z_max = _obj_cfg._sm_param.limit.z_max;
//   double t1 = (z_min - los.pt[2]) / (los.unit_vector[2] + 1e-12);
//   double t2 = (z_max - los.pt[2]) / (los.unit_vector[2] + 1e-12);
//   double t_start = std::min(t1, t2);
//   double t_end = std::max(t1, t2);

//   double step = (t_end - t_start) / n_seg;
//   pts_2d.reserve(n_seg + 1);

//   for (int i = 0; i <= n_seg; ++i) {
//     double t = t_start + i * step;
//     Pt3D p3d = { los.pt[0] + t * los.unit_vector[0],
//                  los.pt[1] + t * los.unit_vector[1],
//                  los.pt[2] + t * los.unit_vector[2] };
//     pts_2d.push_back(_cams[cam_id].project(p3d));
//   }
//   return pts_2d;
// }
...
//   // 小孔模型处理逻辑
//   if (n_seg <= 1) {
//     Pt2D a = _cams[cam_id].project(los.pt);
//     Pt2D b = _cams[cam_id].project(Pt3D{
//         los.pt[0] + los.unit_vector[0],
//         los.pt[1] + los.unit_vector[1],
//         los.pt[2] + los.unit_vector[2]
//     });
...
//       dx /= norm;
//       dy /= norm;
//       // 在二维图像平面向两侧无限延伸（10000像素足以覆盖常规相机画幅）
//       // 确保下游求交逻辑不会因为端点截断而丢弃有效点
//       pts_2d.push_back(Pt2D{a[0] - 10000.0 * dx, a[1] - 10000.0 * dy});
//       pts_2d.push_back(Pt2D{a[0] + 10000.0 * dx, a[1] + 10000.0 * dy});
//     } else {
//       pts_2d.push_back(a);
//       pts_2d.push_back(b);
...
//   // 多项式模型处理逻辑（3D AABB 包围盒求交）
//   double t_min = 0.0;
//   double t_max = std::numeric_limits<double>::max();
//   const auto &lim = _obj_cfg._sm_param.limit;

//   if (std::abs(los.unit_vector[0]) > 1e-12) {
//       double tx1 = (lim.x_min - los.pt[0]) / los.unit_vector[0];
//       double tx2 = (lim.x_max - los.pt[0]) / los.unit_vector[0];
//       t_min = std::max(t_min, std::min(tx1, tx2));
//       t_max = std::min(t_max, std::max(tx1, tx2));
//   }

//   if (std::abs(los.unit_vector[1]) > 1e-12) {
//       double ty1 = (lim.y_min - los.pt[1]) / los.unit_vector[1];
//       double ty2 = (lim.y_max - los.pt[1]) / los.unit_vector[1];
//       t_min = std::max(t_min, std::min(ty1, ty2));
//       t_max = std::min(t_max, std::max(ty1, ty2));
//   }

//   if (std::abs(los.unit_vector[2]) > 1e-12) {
//       double tz1 = (lim.z_min - los.pt[2]) / los.unit_vector[2];
//       double tz2 = (lim.z_max - los.pt[2]) / los.unit_vector[2];
...
// ---- helper: project a set of LOS to 2D polylines on a camera ----
void StereoMatch::buildPolylinesOnCam(const std::vector<Line3D> &los3d,
                                             int cam_id,
                                             std::vector<std::vector<Pt2D>> &out_polylines) const {
  out_polylines.clear();
  out_polylines.reserve(los3d.size());
  for (const auto &L3 : los3d) {
    out_polylines.push_back(makePolylineVerticesFromLOS3D(cam_id, L3));
  }
}
...
// buffer) ----
void StereoMatch::buildLinesOnCam(const std::vector<Line3D> &los3d,
                                         int cam_id,
...
//     // 🌟 核心修改：生成反向投影的折线
//     std::vector<Pt2D> polyline = makePolylineVerticesFromLOS3D(ci, los_t);
//     if (polyline.size() < 2) return false;

//     double min_dist2 = std::numeric_limits<double>::max();

//     // 🌟 核心修改：计算选定点 qi 到反向投影折线的最短距离
//     for (size_t j = 0; j < polyline.size() - 1; ++j) {
//         const Pt2D& A = polyline[j];
//         const Pt2D& B = polyline[j+1];

//         double dx_AB = B[0] - A[0];
//         double dy_AB = B[1] - A[1];
//         double dx_AQ = qi[0] - A[0];
//         double dy_AQ = qi[1] - A[1];

//         double AB_squared = dx_AB * dx_AB + dy_AB * dy_AB;
//         double t = 0.0;
//         if (AB_squared > 1e-12) {
//             t = (dx_AQ * dx_AB + dy_AQ * dy_AB) / AB_squared;
//             t = std::max(0.0, std::min(1.0, t)); // 垂足限制在线段内
//         }

//         double proj_x = A[0] + t * dx_AB;
//         double proj_y = A[1] + t * dy_AB;

//         double d2 = (qi[0] - proj_x) * (qi[0] - proj_x) + (qi[1] - proj_y) * (qi[1] - proj_y);
...
// ===================== 新增：IDMap 折线求交逻辑 =====================
void IDMap::computeStripIntersection(const std::vector<std::vector<Pt2D>> &polylines_px,
                                     double tol_px,
                                     std::vector<RowSpan> &spans) const {
  // 初始为全屏 (准备做 AND 交集)
  spans.assign(_rows_cell, RowSpan{0, _cols_cell - 1});

  const double hw = 0.5 * _cell_px;
  const double hh = 0.5 * _cell_px;
  const double to_index = 1.0 / _cell_px;
  const double eps = 1e-12;
  const double tol_px_grow = tol_px + 1e-6;
  const double eps_c = 1e-9;

  for (const auto &poly : polylines_px) {
    if (poly.size() < 2) continue;

    // =====================================================================
    // 极速路径：小孔模型（退化为单段无限长直线）
    // 完全避免分配 cur_span，并恢复 Early Exit 提早剪枝优化
    // =====================================================================
    if (poly.size() == 2) {
      Pt2D u = myMATH::createUnitVector(poly[0], poly[1]);
      Pt2D n{-u[1], u[0]};
      double d = -(n[0] * poly[0][0] + n[1] * poly[0][1]);
      double proj_extent = std::fabs(n[0]) * hw + std::fabs(n[1]) * hh;
      double T = tol_px_grow + proj_extent;

      for (int cy = 0; cy < _rows_cell; ++cy) {
        RowSpan &row = spans[cy];
        // 🌟 性能核心：提早剪枝，如果这行已经无效，直接跳过数学计算
        if (row.x_min > row.x_max) continue; 

        double y_c = (cy + 0.5) * _cell_px;
        double C = n[1] * y_c + d;
        int a = 0, b = _cols_cell - 1;

        if (std::fabs(n[0]) < eps) {
          if (std::fabs(C) > T) {
            row.x_min = 1; row.x_max = 0;
            continue;
          }
        } else {
          double x_left = (-T - C) / n[0];
      ...
      continue; // 这条极线处理完毕，直接进行下一条
    }

    // =====================================================================
    // 常规路径：多项式模型（多段折线）
    // 需要用到 cur_span 来做局部的 OR 并集
    // =====================================================================
    std::vector<RowSpan> cur_span(_rows_cell, RowSpan{_cols_cell, -1}); 

    for (size_t i = 0; i < poly.size() - 1; ++i) {
      const Pt2D& p1 = poly[i];
      const Pt2D& p2 = poly[i+1];

      double x_min_seg = std::min(p1[0], p2[0]) - tol_px_grow - hw;
      double x_max_seg = std::max(p1[0], p2[0]) + tol_px_grow + hw;
      double y_min_seg = std::min(p1[1], p2[1]) - tol_px_grow - hh;
      double y_max_seg = std::max(p1[1], p2[1]) + tol_px_grow + hh;

      int r_start = std::max(0, static_cast<int>(y_min_seg * to_index));
      int r_end = std::min(_rows_cell - 1, static_cast<int>(y_max_seg * to_index));

      if (r_start > r_end) continue;

      Pt2D u = myMATH::createUnitVector(p1, p2);
      Pt2D n{-u[1], u[0]};
      double d = -(n[0] * p1[0] + n[1] * p1[1]);
      double proj_extent = std::fabs(n[0]) * hw + std::fabs(n[1]) * hh;
      double T = tol_px_grow + proj_extent;

      for (int cy = r_start; cy <= r_end; ++cy) {
        double y_c = (cy + 0.5) * _cell_px;
        double C = n[1] * y_c + d;
        int a = 0, b = _cols_cell - 1;

        if (std::fabs(n[0]) < eps) {
          if (std::fabs(C) > T) continue;
        } else {
          double x_left = (-T - C) / n[0];
          double x_right = (T - C) / n[0];
          if (x_left > x_right) std::swap(x_left, x_right);
        ...
        int cell_x_min = static_cast<int>(std::floor(x_min_seg * to_index));
        int cell_x_max = static_cast<int>(std::floor(x_max_seg * to_index));
        a = std::max({0, a, cell_x_min});
        b = std::min({_cols_cell - 1, b, cell_x_max});

        if (a <= b) {
          cur_span[cy].x_min = std::min(cur_span[cy].x_min, a);
          cur_span[cy].x_max = std::max(cur_span[cy].x_max, b);
        }
      }
    }

    for (int cy = 0; cy < _rows_cell; ++cy) {
      spans[cy].x_min = std::max(spans[cy].x_min, cur_span[cy].x_min);
      spans[cy].x_max = std::min(spans[cy].x_max, cur_span[cy].x_max);
    }
  }
}
// // ===================== 新增：IDMap 点到折线最短距离逻辑 =====================
// void IDMap::visitPointsInRowSpans(const std::vector<RowSpan> &spans,
//                                   const std::vector<std::vector<Pt2D>> &polylines_px,
//                                   double tol_px,
//                                   std::vector<int> &out_indices) const {
//   out_indices.clear();
//   if (!_objs || polylines_px.empty() || spans.empty()) return;
...
//     for (int cx = row.x_min; cx <= row.x_max; ++cx) {
//       for (int pid : _buckets[idx(cy, cx)]) {
...
//         // 必须满足所有相机的视线约束
//         for (const auto &poly : polylines_px) {
//           if (poly.size() < 2) { point_valid = false; break; }
          
//           double min_dist2 = std::numeric_limits<double>::max();
          
//           // 遍历单条视线的所有短线段，求真实的最短几何距离
//           for (size_t i = 0; i < poly.size() - 1; ++i) {
//             const Pt2D& A = poly[i];
//             const Pt2D& B = poly[i+1];

//             double dx_AB = B[0] - A[0];
//             double dy_AB = B[1] - A[1];
//             double dx_AQ = q[0] - A[0];
//             double dy_AQ = q[1] - A[1];

//             double AB_squared = dx_AB * dx_AB + dy_AB * dy_AB;
//             double t = 0.0;
//             if (AB_squared > 1e-12) {
//                 t = (dx_AQ * dx_AB + dy_AQ * dy_AB) / AB_squared;
//                 t = std::max(0.0, std::min(1.0, t)); // 强行把垂足限制在线段内部
//             }

//             double proj_x = A[0] + t * dx_AB;
//             double proj_y = A[1] + t * dy_AB;

//             double d2 = (q[0] - proj_x) * (q[0] - proj_x) + (q[1] - proj_y) * (q[1] - proj_y);
//             if (d2 < min_dist2) min_dist2 = d2;
//           }

//           if (min_dist2 > tol2) {
//             point_valid = false;
//             break; // 只要有一条视线距离超标，直接抛弃该粒子
//           }
//         }
...
void IDMap::visitPointsInRowSpans(const std::vector<RowSpan> &spans,
  ...
  std::vector<Pt2D> fast_U(polylines_px.size(), Pt2D{0.0, 0.0});
  std::vector<bool> is_fast_path(polylines_px.size(), false);
  
  for (size_t i = 0; i < polylines_px.size(); ++i) {
      if (polylines_px[i].size() == 2) {
          is_fast_path[i] = true;
          double dx = polylines_px[i][1][0] - polylines_px[i][0][0];
          double dy = polylines_px[i][1][1] - polylines_px[i][0][1];
          double norm = std::sqrt(dx * dx + dy * dy);
          if (norm > 1e-12) {
              fast_U[i][0] = dx / norm;
              fast_U[i][1] = dy / norm;
  ...
  for (int cy = 0; cy < _rows_cell; ++cy) {
    ...
    for (int cx = row.x_min; cx <= row.x_max; ++cx) {
      for (int pid : _buckets[idx(cy, cx)]) {
        ...
        for (size_t k = 0; k < polylines_px.size(); ++k) {
          const auto &poly = polylines_px[k];
          if (poly.size() < 2) { point_valid = false; break; }
          
          if (is_fast_path[k]) {
              double dx_AQ = q[0] - poly[0][0];
              double dy_AQ = q[1] - poly[0][1];
              double cross = dy_AQ * fast_U[k][0] - dx_AQ * fast_U[k][1];
              if (cross * cross > tol2) {
                  point_valid = false;
                  break;
              }
