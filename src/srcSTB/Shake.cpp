...
double TracerShakeStrategy::project2DInt(const Object3D &obj, int id_cam,
  ...
  // 2. 动态判别：如果有强度信息 (NN检测)，使用个体参数渲染
  if (tr->_intensity > 0.0) {
    // 数组大小为 5：A, B, C, cos(theta), sin(theta)
    static thread_local std::vector<double> otf(5, 0.0);

    // 赋值 A: 峰值强度
    otf[0] = tr->_intensity;

    // 换算 B 和 C，增加极小值保护防止除零
    double r_px = std::max(tr->_r_px, 0.1); 
    double coeff = 2.0 / (r_px * r_px);
    otf[1] = coeff;
    otf[2] = coeff;
    ...
    return gaussIntensity(col, row, tr->_pt_center, otf);
  } 
  else {
    const auto &tr_cfg = static_cast<const TracerConfig &>(_obj_cfg);
    ...
    if (cache.obj != &obj || cache.cam != id_cam || center_changed()) {
      ...
      cache.otf = tr_cfg._otf.getOTFParam(id_cam, obj._pt_center);
    }

    return gaussIntensity(col, row, tr->_pt_center, cache.otf);
  }
}
