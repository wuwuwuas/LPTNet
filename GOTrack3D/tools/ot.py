...
def sinkhorn(feature1, feature2, pcloud1, pcloud2, dis_range, epsilon, gamma, max_iter, candidate):
    ...
    """
    #knn_in_split = KNN(k=32, transpose_mode=True)
    #_, candidate_in_split = knn_in_split(pc_1_orig, points)

    # Force transport to be zero for points further than 10 m apart

    can = torch.zeros(pcloud1.shape[0],pcloud1.shape[1],pcloud2.shape[1]).cuda() #B x N x M
    
    # can2 = torch.zeros(pcloud1.shape[0],pcloud1.shape[1],pcloud2.shape[1]).cuda()
    # candidate2 = candidate[..., 0:32]
    # can2.scatter_(-1, candidate2, 1)
    # support2 = can2
    
    #------
    candidate = candidate[..., 0:dis_range[0]]#B,Np,1
    # candidate = candidate[..., dis_range[0] - 1:dis_range[0]] # no
    
    # if dis_range[0]>2:      # no
    #     candidate = candidate[..., dis_range[0] - 3:dis_range[0]]
    can.scatter_(-1, candidate, 1)
    support = can
    #-------------------
    # print(torch.sum(support, dim=2))
    # print(torch.sum(candidate, dim=2))
    # print(canditate.shape,can.shape,support.shape)
    ...
    C = 1.0 - S #+ distance_matrix / max_d * 100
    
    # print(S.shape, support2.shape)
    # print(torch.sum(support2, dim=2))
    # S = S * support2
