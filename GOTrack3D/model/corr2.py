...
class CorrBlock2(nn.Module):
    ...
    def init_module(self, fmap1, fmap2, xyz2, transport):
        ...
        xyz2 = xyz2.view(b, 1, n_p2, 3).expand(b, n_p1, n_p2, 3)

        corr_topk = torch.topk(corr.clone(), k=min(n_p1, n_p2, 2000), dim=2, sorted=True)
        self.truncated_corr = corr_topk.values
        indx = corr_topk.indices.reshape(b, n_p1, min(n_p1, n_p2, 2000), 1).expand(b, n_p1, min(n_p1, n_p2, 2000), 3)
        self.ones_matrix = torch.ones_like(self.truncated_corr)
        
        self.truncate_xyz2 = torch.gather(xyz2, dim=2, index=indx)
