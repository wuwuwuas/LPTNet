...
class GeoSetConv(torch.nn.Module):
    def __init__(self, nb_feat_in, nb_feat_out):
        ...
        self.gn1 = torch.nn.GroupNorm(8, mid_feature, affine=True) # 组归一化
        ...
        self.pool = lambda x: torch.max(x, 2)[0]# 第二个维度最大池化
        self.lrelu = torch.nn.LeakyReLU(negative_slope=0.1)
    ...
    def forward(self, signal, graph):
        ...
        # Concatenate input features with edge features
        signal = signal.reshape(b * n, c)
        edge_feature = signal[graph.edges].reshape(-1, graph.k_neighbors, c) - signal.view(b * n, 1, c) # 构建的图的最近邻点的相对坐标   ？？？？？？？？？？？？？？？        下一句torch.cat之后重复了吧
        signal = torch.cat((edge_feature.view(-1, c), graph.edge_feats), -1)
        signal = signal.view(b, n_out, graph.k_neighbors, c + 6) # 邻居点的相对坐标，邻居点之间的相对坐标，边的长度、角度等特征
        signal = signal.transpose(1, -1) # 把不同的特征放在不同的通道里
        ...
        return signal.transpose(1, -1) # B N NUM C